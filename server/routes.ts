...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  apiRouter.post("/oauth/exchange", async (req, res) => {
    try {
        ...
        const tokenResponse = await fetch(
          "https://oauth2.googleapis.com/token",
          {
            ...
            body: new URLSearchParams({
              ...
              redirect_uri:
                "https://66793246-3db9-4ceb-9826-7a03fb6463f5-00-tjsgi59cx3ud.worf.replit:3001/auth/callback",
      ...
      return res
        .status(500)
        .json({ success: false, message: "Server error exchanging token" });
    }
  });
    ...
    try {
      ...
      if (provider && typeof provider === "string") {
        ...
        if (provider === "airtable" || provider === "google") {
          ...
          if (success) {
            console.log(
              `Successfully deleted ${provider} token for user ${userId}`,
            );
          } else {
    ...
    try {
      ...
      if (!token) {
        return res
          .status(401)
          .json({
            message: "Airtable token not found. Please connect Airtable first.",
          });
      }
  ...
  apiRouter.get("/airtable/bases/:baseId/tables", async (req, res) => {
    try {
      ...
      console.error(
        `Error listing tables for base ${req.params.baseId}:`,
        error,
      );
      res.status(500).json({
      ...
      try {
        ...
        if (!token) {
          return res
            ...
            .json({
              message:
                "Airtable token not found. Please connect Airtable first.",
            });
        }
        ...
        console.error(
          `Error getting fields for table ${req.params.tableId}:`,
          error,
        );
        res.status(500).json({
    ...
    try {
      ...
      if (!token) {
        return res
          ...
          .json({
            message:
              "Google token not found. Please connect Google Sheets first.",
          });
      }
  ...
  apiRouter.get(
    "/google/spreadsheets/:spreadsheetId/sheets",
    async (req, res) => {
      try {
        ...
        console.error(
          `Error listing sheets for spreadsheet ${req.params.spreadsheetId}:`,
          error,
        );
        res.status(500).json({
  ...
  apiRouter.get(
    "/google/spreadsheets/:spreadsheetId/sheets/:sheetName/headers",
    async (req, res) => {
      try {
        ...
        const headers = await GoogleSheetsService.getSheetHeaders(
          token,
          spreadsheetId,
          sheetName,
        );
        ...
        console.error(
          `Error getting headers for sheet ${req.params.sheetName}:`,
          error,
        );
        res.status(500).json({
  ...
  apiRouter.post("/oauth/token", async (req, res) => {
    try {
      ...
      console.log(
        `OAuth token saved for provider ${token.provider} with ID ${token.id}`,
      );
  ...
  apiRouter.get(
    ...
    async (req, res) => {
      try {
        ...
        res
          .status(500)
          .json({ message: "Failed to fetch Google Sheet headers" });
      }
    },
  );
  ...
  apiRouter.post("/integrations", async (req, res) => {
    try {
      ...
      if (configData.createNewAirtableBase) {
        const base = await AirtableService.createBase(
          airtableToken,
          `${configData.name} - Base`,
        );
      ...
      const airtableConnection =
        await makeService.createAirtableConnection(airtableToken);
      const googleConnection =
        await makeService.createGoogleSheetsConnection(googleToken);
