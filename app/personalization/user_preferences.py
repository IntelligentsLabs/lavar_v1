...
def get_user_interaction_context(user_id: str) -> str:
    ...
    try:
        ...
        voice_response = (supabase.table("voice_interactions").select(
            "agent_response, timestamp").eq(
                "user_id", int(user_id.replace("-", "")[:8],
                               16)).order("timestamp",
                                          desc=True).limit(5).execute())
        if voice_response.data:
            for row in voice_response.data:
                timestamp = row.get("timestamp")
                agent_response = row.get("agent_response")
                context_entries.append(
                    f"At {timestamp}, agent said: {agent_response}")

        # Query recent user book interactions (for example, any notes the user made).
        book_response = (supabase.table(
            "user_book_interactions").select("notes, start_time, end_time").eq(
                "user_id", user_id).order("start_time",
                                          desc=True).limit(3).execute())
        ...
        print(
            f"Error fetching user interaction context for user {user_id}: {e}")
...
def get_user_preferences(user_id: str) -> Dict[str, str]:
    ...
    try:
        ...
        if agent_response.data and len(agent_response.data) > 0:
            ...
            preferences["speaking_rate"] = str(
                row.get("speaking_rate", "normal"))
            preferences["interaction_style"] = row.get("interaction_style",
                                                       "friendly")
            preferences["explanation_detail_level"] = row.get(
                "explanation_detail_level", "standard")
            preferences["discussion_depth"] = row.get("discussion_depth",
                                                      "moderate")
        ...
        if cognitive_response.data and len(cognitive_response.data) > 0:
            ...
            preferences["reading_pace"] = str(row.get("reading_pace",
                                                      "normal"))
            preferences["preferred_complexity_level"] = str(
                row.get("preferred_complexity_level", "medium"))
            preferences["preferred_interaction_frequency"] = row.get(
